...
type DataContextType = {
  ...
  isSearching: boolean;
  searchToursFromGallery: (skipAutoSearch?: boolean) => Promise<void>;
};
...
export const DataProvider = ({ children }: { children: ReactNode }) => {
  ...
  const searchToursFromGallery = useCallback(
    async (skipAutoSearch = false) => {
      if (!params.param1 || !params.param2) {
        return;
      }

      // Если skipAutoSearch = true, только обновляем параметры без запуска поиска
      if (skipAutoSearch) {
        console.log("Skipping auto search from gallery, parameters updated");
        return;
      }
